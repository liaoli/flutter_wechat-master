...
extension LcConversationExt on Conversation {
  ...
  String title({bool showMember = false}) {
    if (isSingle) {
      ...
      return name != null
          ? (showMember ? name! + '(${members?.length ?? 0})' : name!)
          : 'title';
    }
  }
  ...
  List<String> get membersStr =>
      members?.map<String>((e) => e.toString()).toList() ?? [];
}
...
extension LCMessageExt on Message {
  ...
  String get contentText {
    ...
    if (this is RedPacketMessage) {
      return Ids.item_red_packet.str() +
          ((this as RedPacketMessage).text ?? '');
    }
    return '';
  }
}
